...
-- 删除旧策略（如果存在）并创建新策略
DROP POLICY IF EXISTS "Allow all operations via edge function" ON gold_prices;
...
-- 删除旧策略（如果存在）并创建新策略
DROP POLICY IF EXISTS "Allow all operations via edge function" ON email_settings;
...
-- 删除旧策略（如果存在）并创建新策略
DROP POLICY IF EXISTS "Allow all operations via edge function" ON data_sources;
...
CREATE TABLE IF NOT EXISTS email_logs (
  ...
  email_type TEXT NOT NULL CHECK (email_type IN ('daily_report', 'price_alert', 'test', 'error')),
...
-- 删除旧策略（如果存在）并创建新策略
DROP POLICY IF EXISTS "Allow all operations via edge function" ON email_logs;
...
INSERT INTO email_settings (user_email, push_frequency, price_threshold, monitored_brands, is_active)
VALUES ('y15205207533@163.com', 'daily', 2.0, ARRAY['周大福', '周生生', '老凤祥'], TRUE)
ON CONFLICT (user_email) DO NOTHING;
